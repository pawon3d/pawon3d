...
[--fx-flyout-translate:translateX(-50px)] border-r',
default => 'fixed m-0 p-8 max-h-dvh min-h-dvh md:[:where(&)]:min-w-[25rem] overflow-y-auto ml-auto
[--fx-flyout-translate:translateX(50px)] border-l',
},
...
// Support <flux:modal ... @close="?"> syntax...
    ...
    // Support <flux:modal... @cancel="?"> syntax...
        ...
        [ $styleAttributes, $attributes ] = Flux::splitAttributes($attributes, ['class', 'style', 'wire:close',
        'x-on:close', 'wire:cancel', 'x-on:cancel']);
        @endphp
        ...
        <ui-modal {{ $attributes }} data-flux-modal>
            ...
            <dialog wire:ignore.self {{-- This needs to be here because the dialog element adds a "close" attribute that
                isn't durable... --}} {{ $styleAttributes->class($classes) }}
                ...
                "
                @else
                x-on:modal-show.document="if ($event.detail.name === @js($name) && (! $event.detail.scope))
                $el.showModal()"
                x-on:modal-close.document="if (! $event.detail.name || ($event.detail.name === @js($name) && (!
                $event.detail.scope))) $el.close()" @endif>
                {{ $slot }}
                ...
                <div class="absolute top-0 right-0 mt-4 mr-4">
                    <flux:modal.close>
                        <flux:button variant="ghost" icon="x-mark" size="sm" alt="Close modal"
                            class="text-zinc-400! hover:text-zinc-800!  "></flux:button>
                    </flux:modal.close>
                </div>
                <?php endif; ?>
            </dialog>
