...
<flux:with-field :$attributes :$name>
    <div {{ $attributes->only('class')->class('w-full relative block group/input') }} data-flux-input>
        ...
        <div
            class="z-10 pointer-events-none absolute top-0 bottom-0 flex items-center justify-center text-xs text-zinc-400/75 pl-3 left-0">
            <flux:icon :icon="$iconLeading" :variant="$iconVariant" :class="$iconClasses" />
        </div>
        <?php elseif ($iconLeading): ?>
        <div
            {{ $iconLeading->attributes->class('z-10 absolute top-0 bottom-0 flex items-center justify-center text-xs text-zinc-400/75 pl-3 left-0') }}>
            {{ $iconLeading }}
        </div>
        <?php endif; ?>
        ...
        <input type="{{ $type }}" {{-- Leave file inputs unstyled... --}}
            ...
            @if (is_numeric($size)) size="{{ $size }}" @endif data-flux-control
            data-flux-group-target>

        <?php if ($kbd): ?>
        <div
            class="pointer-events-none absolute top-0 bottom-0 flex items-center justify-center text-xs text-zinc-400 pr-4 right-0">
            {{ $kbd }}
        </div>
        <?php endif; ?>
        ...
        <div
            class="pointer-events-none absolute top-0 bottom-0 flex items-center justify-center text-xs text-zinc-400/75 pr-3 right-0">
            <flux:icon :icon="$iconTrailing" :variant="$iconVariant" :class="$iconClasses" />
        </div>
        <?php elseif ($iconTrailing): ?>
        <div
            {{ $iconTrailing->attributes->class('absolute top-0 bottom-0 flex items-center justify-center text-xs text-zinc-400/75 pr-2 right-0') }}>
            {{ $iconTrailing }}
        </div>
        <?php endif; ?>
...
<button {{ $attributes->merge(['type' => 'button'])->class([$classes, 'w-full relative flex']) }}>
    ...
    <div class="block self-center text-left flex-1 font-medium text-zinc-400 ">
        {{ $attributes->get('placeholder') }}
    </div>
    <?php else: ?>
    <div class="text-left self-center flex-1 font-medium text-zinc-800 ">
        {{ $slot }}
    </div>
    <?php endif; ?>
