<div>
    ...
    <div class="mb-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div class="flex gap-2 items-center w-full sm:w-auto justify-end">
            <flux:button variant="secondary" wire:click="riwayatPembaruan" class="w-full sm:w-auto">Riwayat Pembaruan
            </flux:button>
        </div>
        @endif
    </div>
    ...
    <div class="mt-8 bg-[#FAFAFA] shadow-sm rounded-2xl p-8">
        ...
        <div class="w-full mt-10 flex flex-col gap-4">
            ...
            <x-form.multi-select
                :options="$categoryOptions->map(fn($c) => ['id' => $c->id, 'name' => $c->name])->toArray()"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" min="0" step="0.01"
                                wire:model.live.debounce.300ms="product_compositions.{{ $index }}.material_quantity"
                                placeholder="0"
                    ...
                    <div class="block md:hidden space-y-4">
                        ...
                        <div class="p-4 bg-white rounded-xl border border-[#D4D4D4] space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-[#ADADAD] uppercase tracking-wider mb-1">Jumlah</label>
                                    <span class="text-sm font-medium text-[#666666]">{{ $batch['quantity'] }} {{
                                        $batch['unit_alias'] }}</span>
                                </div>
                                <div class="text-right">
                                    <label
                                        class="block text-[10px] font-bold text-[#ADADAD] uppercase tracking-wider mb-1">Jumlah
                                        (Utama)</label>
                                    <span class="text-sm font-medium text-[#666666]">{{ $batch['main_quantity'] }} {{
                                        $soloInventory['main_unit_alias'] }}</span>
                                </div>
                            </div>
                            <div class="flex justify-between items-center pt-1">
                                <span class="text-xs font-bold text-[#666666]">Tanggal Expired</span>
                                <span class="text-sm font-medium text-[#666666]">{{
                                    \Carbon\Carbon::parse($batch['date'])->format('d M Y') }}</span>
                            </div>
                        </div>
                        @empty
                        ...
                        <div
                            class="p-4 bg-[#EAEAEA] rounded-xl border border-[#D4D4D4] flex justify-between items-center">
                    ...
                    <div class="hidden md:block w-full overflow-x-auto rounded-2xl shadow-sm">
                        <table class="w-full text-sm">
                            ...
                            <tbody>
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-center text-[#959595] font-medium">Tidak ada
                                        data batch</td>
                                </tr>
                                @endforelse
                            </tbody>
    ...
    <div class="mt-8 bg-[#FAFAFA] shadow-sm rounded-2xl p-6 sm:p-8">
        <div class="w-full flex flex-col gap-4">
            ...
            @if ($is_other)
            <div class="flex flex-col sm:flex-row justify-end gap-4">
                <flux:button variant="primary" icon="bolt" href="{{ route('jenis-biaya') }}" wire:navigate
                    class="w-full sm:w-auto">
                ...
                <div class="p-4 bg-white rounded-xl border border-[#D4D4D4] space-y-4">
                    <div class="flex justify-between items-start">
                        <div class="flex-1">
                            ...
                            <select wire:model.defer="other_costs.{{ $index }}.type_cost_id"
                                class="w-full bg-transparent border-0 border-b border-[#D4D4D4] focus:border-[#74512D] focus:outline-none focus:ring-0 text-[#666666] font-medium appearance-none pr-8">
                        ...
                        <button type="button" wire:click.prevent="removeOther({{ $index }})"
                            class="ml-2 inline-flex items-center justify-center w-8 h-8 text-[#666666] hover:text-red-600 transition-colors">
                            <svg class="w-4 h-5" fill="currentColor" viewBox="0 0 12 16">
                                <path
                                    d="M11 2H8.5L7.5 1H4.5L3.5 2H1V4H11V2ZM2 14C2 15.1 2.9 16 4 16H8C9.1 16 10 15.1 10 14V5H2V14Z" />
                        ...
                        <tr class="h-[60px] border-b border-[#D4D4D4] bg-[#FAFAFA]">
                            ...
                            <td class="px-6 text-right">
                                <input type="number" min="0"
                                    wire:model.live.debounce.300ms="other_costs.{{ $index }}.price" placeholder="0"
                                    class="w-full max-w-[190px] px-2.5 py-1.5 bg-[#FAFAFA] border border-[#ADADAD] rounded-md text-right text-[#666666] font-medium focus:outline-none focus:border-[#74512D]" />
                            </td>
                            <td class="px-6 text-center">
                                <button type="button" wire:click.prevent="removeOther({{ $index }})"
    ...
    <div class="mt-8 bg-[#FAFAFA] shadow-sm rounded-2xl p-6 sm:p-8">
        <div class="w-full flex flex-col gap-4">
            ...
            <input type="number" min="1" wire:model.live.debounce.300ms="pcs" placeholder="1"
            ...
            <div class="block md:hidden space-y-4">
                <div class="p-4 bg-white rounded-xl border border-[#D4D4D4] space-y-4">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-[#666666]">Suhu Ruangan <span
                                class="font-normal text-xs">(20–25°C)</span></span>
                        <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center justify-between pt-2 border-t border-dashed border-[#D4D4D4]">
                        <span class="text-sm font-medium text-[#666666]">Suhu Dingin <span
                                class="font-normal text-xs">(4–8°C)</span></span>
                        <div class="flex items-center gap-2">
                    ...
                    <div class="flex items-center justify-between pt-2 border-t border-dashed border-[#D4D4D4]">
                        <span class="text-sm font-medium text-[#666666]">Suhu Beku <span
                                class="font-normal text-xs">(-18°C+)</span></span>
                        <div class="flex items-center gap-2">
    ...
    <div class="mt-8 bg-[#FAFAFA] shadow-sm rounded-2xl p-6 sm:p-8">
        <div class="w-full flex md:flex-row flex-col gap-8">
                    ...
                    <div class="block md:hidden space-y-4">
                        <div class="p-4 bg-white rounded-xl border border-[#D4D4D4] space-y-4">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
                                ...
                                <span class="text-sm font-bold text-[#666666]">Rp{{ number_format($capital, 0, ',', '.')
                                    }}</span>
                            </div>
                            <div
                                class="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4 pt-2 border-t border-dashed border-[#D4D4D4]">
                                <span class="text-sm font-medium text-[#666666]">Modal per PCS</span>
                                <span class="text-sm font-bold text-[#666666]">Rp{{ number_format($pcs_capital, 0, ',',
                                    '.') }}</span>
                            </div>
                            @else
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-4">
            ...
            <div class="md:w-1/2 flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-4">
                    @foreach (['pesanan-reguler' => 'Pesanan Reguler', 'pesanan-kotak' => 'Pesanan Kotak', 'siap-beli'
                    => 'Siap Saji'] as $value => $label)
