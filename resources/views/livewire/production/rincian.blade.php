    ...
    <div class="flex flex-col gap-[30px] sm:gap-[50px] items-end">
                        ...
                        <div class="flex flex-col min-w-[900px]">
                            ...
                            <div class="flex items-center w-full h-[60px] overflow-hidden">
                                ...
                                <div class="w-[135px] bg-[#eaeaea] px-[25px] h-full flex items-center justify-end">
                                    <p
                                        style="font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 14px; color: #666666; text-align: right; white-space: nowrap;">
                                        @php $totalDiff = $production_details->sum(fn($d) => $d->quantity_get -
                                        $d->quantity_plan); @endphp
                                        @if ($totalDiff > 0)
                                        +{{ $totalDiff }}
                                        @else
                                        {{ $totalDiff }}
                                ...
                                <div class="w-[115px] bg-[#eaeaea] px-[25px] h-full flex items-center justify-end">
                                    <p
                                        style="font-family: 'Montserrat', sans-serif; font-weight: 700; font-size: 14px; color: #666666; text-align: right; white-space: nowrap;">
                                        @php $totalExcess = $production_details->sum(fn($d) => max(0, $d->quantity_get -
                                        $d->quantity_plan)); @endphp
                                        {{ $totalExcess }}
                                    </p>
                                </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 sm:gap-[30px] items-center justify-end w-full">
            <flux:button variant="outline" icon="check-circle" wire:click="finish"
                ...
                <span class="font-semibold text-[16px] text-[#3f4e4f]"
                    style="font-family: 'Montserrat', sans-serif;">Selesaikan Produksi</span>
            </flux:button>
            @if ($total_quantity_get < $total_quantity_plan) <flux:button
                href="{{ route('produksi.mulai', $production->id) }}" variant="secondary" icon="clipboard-document-list"
                wire:navigate
                ...
                <span class="font-semibold text-[16px] text-[#f8f4e1]"
                    style="font-family: 'Montserrat', sans-serif;">Dapatkan Produk</span>
                </flux:button>
                @endif
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 sm:gap-[30px] items-center justify-end w-full">
            <button wire:click="confirmDelete"
                ...
                <span class="sm:hidden font-semibold text-[16px] text-white"
                    style="font-family: 'Montserrat', sans-serif;">Hapus Rencana</span>
            </button>
            @if ($production->method != 'siap-beli' && $total_quantity_get <= 0) <a
                href="{{ route('produksi.edit-produksi-pesanan', $production->id) }}" wire:navigate
                ...
                <span class="font-semibold text-[16px] text-[#f6f6f6]"
                    style="font-family: 'Montserrat', sans-serif;">Ubah Rencana</span>
                </a>
                @endif
                <button wire:click="start"
                    ...
                    <span class="font-semibold text-[16px] text-[#f8f4e1]"
                        style="font-family: 'Montserrat', sans-serif;">Mulai Produksi</span>
                </button>
        </div>
        @endif
    </div>
