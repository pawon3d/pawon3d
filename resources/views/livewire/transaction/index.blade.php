<div class="h-full">
    ...
    <div class="px-4 sm:px-0 flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        <h1 class="text-xl font-semibold text-[#666666] text-center sm:text-left"
            style="font-family: 'Montserrat', sans-serif;">Daftar Menu
        </h1>
        <div class="flex flex-col sm:flex-row gap-2 items-center w-full sm:w-auto">
            ...
            <flux:button type="button" variant="secondary" href="{{ route('transaksi.riwayat-sesi') }}" wire:navigate
    ...
    <x-alert.info style="font-family: 'Montserrat', sans-serif; font-weight: 600; font-size: 14px; line-height: 1.5;">
        <p class="mb-2">
    ...
    <div class="px-4 sm:px-0 flex flex-col sm:flex-row sm:h-[120px] mb-8 gap-1 sm:gap-0">
        <button wire:click="$set('method', 'pesanan-reguler')" class="flex-1 bg-[#fafafa] flex flex-col items-center justify-center gap-1 rounded-[15px] sm:rounded-none sm:rounded-tl-[15px] sm:rounded-bl-[15px] shadow-sm transition-all py-4 sm:py-0
        ...
        <button wire:click="$set('method', 'pesanan-kotak')" class="flex-1 bg-[#fafafa] flex flex-col items-center justify-center gap-1 sm:rounded-none shadow-sm transition-all py-4 sm:py-0
        ...
        <button wire:click="$set('method', 'siap-beli')" class="flex-1 bg-[#fafafa] flex flex-col items-center justify-center gap-1 rounded-[15px] sm:rounded-none sm:rounded-tr-[15px] sm:rounded-br-[15px] shadow-sm transition-all py-4 sm:py-0
    ...
    <div class="bg-[#fafafa] rounded-[15px] shadow-sm p-4 sm:p-8">
        ...
        <div class="flex flex-col lg:flex-row justify-between lg:items-center mb-8 gap-4 flex-wrap">
            <div class="flex flex-col sm:flex-row gap-4 items-center flex-1 w-full lg:max-w-[550px]">
                ...
                <button
                    class="flex items-center gap-2 text-[#666666] hover:text-[#74512d] transition-colors justify-center">
                    <flux:icon icon="funnel" class="size-[25px]" />
                    <span class="text-[16px] font-medium" style="font-family: 'Montserrat', sans-serif;">Filter</span>
                </button>
            </div>

            <div class="flex gap-2 items-center flex-wrap justify-center w-full lg:w-auto">
                @if ($method != 'siap-beli')
                <flux:button type="button" variant="primary" class="flex-1 sm:flex-none"
                    href="{{ route('transaksi.pesanan', ['method' => $method]) }}" icon="clipboard" wire:navigate>
                    <span>Daftar Pesanan</span>
                </flux:button>
                @else
                <flux:button type="button" variant="primary" href="{{ route('transaksi.siap-beli') }}"
                    class="flex-1 sm:flex-none" icon="clipboard" wire:navigate>
                    <span>Daftar Produk</span>
                </flux:button>
                @endif
                <flux:button type="button" variant="primary" icon="history" class="flex-none"
        ...
        <div class="flex flex-col xl:flex-row gap-[20px] @if (count($cart) > 0) justify-between @endif">
            ...
            <div class="@if (count($cart) > 0) flex-1 @else w-full @endif">
                <div @class([ 'grid gap-8 min-h-[500px] pr-2' , 'grid-cols-2 lg:grid-cols-3'=> count($cart) > 0,
                    ...
                    <div class="flex flex-col gap-5 pb-6">
                        ...
                        <div class="flex items-center justify-center gap-[11px] w-full">
                            ...
                            <button wire:click="decrementItem('{{ $product->id }}')"
                                ...
                                <flux:icon icon="minus" class="size-[18px] text-[#74512d] group-hover:text-white" />
                            </button>
                            ...
                            <button wire:click="incrementItem('{{ $product->id }}')"
                                class="border-[1.5px] border-[#74512d] rounded-[30px] w-[35px] h-[35px] flex items-center justify-center hover:bg-[#74512d] transition-colors group">
                                <flux:icon icon="plus" class="size-[18px] text-[#74512d] group-hover:text-white" />
                            </button>
                        </div>
                        @else
                    ...
                    @empty
                    <div @class([ 'bg-[#eaeaea] rounded-[15px] p-8 text-center min-h-[385px] flex flex-col items-center justify-center'
            ...
            <div class="w-full xl:w-[420px] flex-shrink-0">
                <div
                                        ...
                                        <div class="flex items-center gap-[15px] sm:gap-[25px]">
                                            ...
                                            <div class="flex items-center gap-[10px] sm:gap-[11px] order-1 sm:order-2">
                                                ...
                                                <button wire:click="decrementItem('{{ $itemId }}')"
                                                    ...
                                                    <flux:icon icon="minus" class="size-[18px] text-[#666666]" />
                                                </button>
                                                ...
                                                <button wire:click="incrementItem('{{ $itemId }}')"
                                                    class="border-[1.5px] border-[#74512d] rounded-[30px] w-[30px] h-[30px] flex items-center justify-center hover:bg-[#74512d] hover:text-white transition-colors">
                                                    <flux:icon icon="plus" class="size-[18px] text-[#666666]" />
                                                </button>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="bg-white border-t border-[#d4d4d4] py-[20px] flex flex-col gap-[30px]">
                        ...
                        {{-- Action Buttons --}}
                        <div class="flex justify-between gap-[10px] px-[20px]">
                            {{-- Cancel Button --}}
                            <button wire:click="clearCart"
                                class="bg-[#c4c4c4] rounded-[15px] px-[25px] py-[10px] flex items-center justify-center gap-[5px] shadow-sm hover:bg-[#b0b0b0] transition-colors">
                            ...
                            {{-- Checkout Button --}}
                            <flux:button variant="secondary" icon="shopping-cart" wire:click="checkout" wire:navigate>
    ...
    <div class="px-4 sm:px-0 flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        <h1 class="text-xl font-semibold text-[#666666] text-center sm:text-left"
            style="font-family: 'Montserrat', sans-serif;">Daftar
            Menu
        </h1>
        <div class="flex flex-col sm:flex-row gap-2 items-center w-full sm:w-auto">
    ...
    <flux:modal name="open-shift-modal" class="w-full max-w-md" wire:model="openShiftModal">
        ...
        <div class="mt-6 flex justify-end space-x-2">
            ...
            <flux:button type="button" icon="play" iconVariant="solid" variant="secondary" wire:click="openShift">Buka
            </flux:button>
        </div>
    </flux:modal>
                    ...
                    <div class="flex-1 overflow-y-auto px-4 py-8 sm:px-[30px] sm:py-[40px]">
                        ...
                        <div class="flex flex-col gap-[15px] mt-2">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-[#666666] font-normal text-[16px]"
                                    style="font-family: 'Montserrat', sans-serif; line-height: 1;">
                                    Rp{{ number_format($receivedCash + $receivedNonCash - $refundTotal - $discountToday,
                                    0, ',', '.') }}
                                </span>
                            </div>
                        </div>
            ...
            <div class="p-4">
                ...
                <div class="flex items-center justify-between mt-2 mb-2">
                    <span class="text-sm text-gray-500 flex items-center">Penerimaan Non Tunai
                        <flux:button icon="information-circle" iconVariant="outline" loading="false" variant="ghost"
                    ...
                    @endphp
                    <span class="text-sm text-gray-500">Rp{{ number_format($transactionPenerimaanNonTunai, 0, ',', '.')
                        }}</span>
                </div>
                <div class="flex items-center justify-between mt-2 mb-2">
                    ...
                    @endphp
                    <span class="text-sm text-gray-500">Rp{{ number_format($transactionRefund, 0, ',', '.') }}</span>
                </div>
                <div class="flex items-center justify-between mt-2 mb-2">
                ...
                <div class="flex items-center justify-between mt-2 mb-2">
                    ...
                    <span class="text-sm text-gray-500">Rp{{ number_format($selectedShift->initial_cash +
                        $transactionPenerimaan - $transactionRefund, 0, ',', '.') }}</span>
                </div>
                <div class="flex items-center justify-between mt-2 mb-2">
                ...
                <div class="flex items-center justify-between mt-2 mb-2">
                    ...
                    <span class="text-sm text-gray-500">Rp{{ number_format(
                        $selectedShift->final_cash - ($selectedShift->initial_cash + $transactionPenerimaan -
                        $transactionRefund),
                        0,
                        ',',
                        '.',
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-3 py-2 whitespace-nowrap capitalize">
                                    {{ $detail->channel->group ? ucfirst($detail->channel->group) :
                                    ucfirst($detail->channel->type) }}
