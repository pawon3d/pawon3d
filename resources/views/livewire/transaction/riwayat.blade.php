<div class="px-4 sm:px-0">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-[32px] mb-8">
        <div class="flex flex-col sm:flex-row gap-4 items-center w-full sm:w-auto">
            ...
            <p class="font-semibold text-[20px] text-[#666666] text-center sm:text-left"
    ...
    <div class="bg-[#fafafa] rounded-[15px] shadow-sm p-4 sm:px-[30px] sm:py-[25px]">
        ...
        <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-[20px] gap-4">
            <div class="flex-1 flex flex-col sm:flex-row gap-[15px] items-center w-full">
                {{-- Search Input --}}
                <div
                    class="flex-1 bg-white border border-[#666666] rounded-[20px] px-[15px] py-0 flex items-center w-full">
        ...
        <div class="overflow-x-auto">
            <div class="min-w-[1000px]">
                <x-table.paginated :headers="$headers" :paginator="$transactions"
                    emptyMessage="Belum ada riwayat transaksi." headerBg="#3f4e4f" headerText="#f8f4e1" bodyBg="#fafafa"
                    bodyText="#666666" wrapperClass="rounded-[15px] border border-[#d4d4d4]">
                    ...
                    <tr class="border-b border-[#d4d4d4] hover:bg-[#f0f0f0] transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <p class="font-medium text-[14px] text-[#666666] truncate max-w-md"
                                style="font-family: 'Montserrat', sans-serif;">
                                {{ $transaction->details->count() > 0 ? $transaction->details->map(fn($d) =>
                                $d->product?->name)->filter()->implode(', ') : 'Tidak ada produk' }}
                            </p>
                        </td>
